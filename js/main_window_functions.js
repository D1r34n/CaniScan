  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (deleteSelected) {
        deleteSelected.addEventListener('click', async (event) => {
            ...
            const confirmDelete = confirm(`Delete ${selectedImageFilenames.size} selected image(s)? This cannot be undone.`);
            if (!confirmDelete) return;

            try {
                const filenames = Array.from(selectedImageFilenames);
                for (const filename of filenames) {
                    const resp = await fetch(`http://localhost:5001/images/${encodeURIComponent(filename)}`, {
                        method: 'DELETE'
                    });
                    if (!resp.ok) {
                        console.error('Failed to delete', filename, await resp.text());
                    }
                }
                // Refresh gallery
    ...
    // Analysis history functionality - now persisted in localStorage
    ...
    function setupGalleryUpload() {
        ...
        uploadInput.addEventListener('change', async (e) => {
            ...
            try {
                const formData = new FormData();
                // Desktop server expects the field name 'image'
                ...
                if (typeof loadGalleryImages === 'function') {
                    setTimeout(() => loadGalleryImages(), 300);
                }
    ...
    // Initialize gallery upload on DOM ready
    document.addEventListener('DOMContentLoaded', () => {
        setupGalleryUpload();
    });
